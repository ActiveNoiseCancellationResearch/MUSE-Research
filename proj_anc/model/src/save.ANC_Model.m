...
di_data = di_data(:,1);
n_samps = min(10000,numel(ui_data));  
...
for n=1:n_samps
    % Pass undesired signal through loudspeaker
    x__(n) = ls_model.new_sample(ui_data(n));
    
    % Model output of loudspeaker through room acoustics
    x_(n) = ra_model.new_sample(x__(n));
    
    % Compute current canceller output
    y(n) = canceller.new_sample(ui_data(n));
    
    % Pass canceller output through headphone speaker
    c(n) = hs_model.new_sample(y(n));
    
    % Get current desired sound sample
    d(n) = di_data(n);
    
    % Compute the sound experienced by the listener
    e(n) = x_(n) + d(n) + c(n);
    
    % Compute just the net undesired sound experienced by the listener
    err(n) = x_(n) + c(n);
    
    % Pass sound at listener through microphone
    e_(n) = mic_model.new_sample(e(n));
    
    % Update canceller based on sound picked up at listener (try to force to zero)
...
% Compute mean squared error, with a moving average
% smoother to generate a useful trend for human interpretation
...
subplot (4,2,6);
plot(c)
title('Canceller Signal at the Listener')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
subplot (4,2,8);
plot(e_)
