classdef ANC_Canceller < handle
    ...
    methods
        function cancel_constructor = ANC_Canceller(filter_order, mu, mic_coeffs, hs_coeffs)
            ...
            cancel_constructor.model_coeffs = zeros(size(filter_order));
            cancel_constructor.model_coeffs(1) = 1;
            cancel_constructor.recent_samps_in = zeros(size(filter_order));
            cancel_constructor.recent_samps_v = zeros(size(filter_order));
        end
   ...
   methods(Static) 
        function coeff_update(my_canceller, e)
            for i = 1:size(my_canceller.model_coeffs)                             
                my_canceller.model_coeffs(i) = my_canceller.model_coeffs(i) - my_canceller.mu...
